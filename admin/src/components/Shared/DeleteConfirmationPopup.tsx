...
export default function DeleteConfirmationPopup({
  ...
  subtitle = "After deleting it cannot be recovered so be careful before deleting it!",
  itemId,
  apiEndpoint,
  previewComponent,
  entityName = "item",
}: DeleteConfirmationPopupProps): ReactElement {
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    // If apiEndpoint and itemId are provided, perform the delete API call
    if (apiEndpoint && itemId) {
      try {
        setLoading(true);
        
        // Normalize apiEndpoint to avoid double slashes
      ...
      // Just trigger callbacks without API call
      onConfirm?.();
      onClose?.();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div
        data-layer="Delete Confirmation Popup"
      ...
      >
        <div className="self-stretch inline-flex justify-center items-start gap-6">
          <div className="flex-1 flex justify-between items-center">
            <div className="flex-1 inline-flex flex-col justify-start items-start gap-3">
              <div className="self-stretch text-center justify-center text-neutral-50 text-lg font-semibold font-['Plus_Jakarta_Sans'] capitalize leading-6">
                {title}
              </div>
              <div className="self-stretch text-center justify-center text-zinc-300 text-sm font-normal font-['Poppins'] leading-5">
                {subtitle}
              </div>
            </div>
          </div>
        ...
        <div className="self-stretch flex flex-col justify-start items-center gap-6">
          <div className="self-stretch flex flex-col justify-start items-start gap-4">
            <div className="self-stretch p-2 bg-zinc-700 rounded-xl inline-flex justify-start items-center gap-2">
              <svg
              ...
              <div className="flex-1 justify-start text-neutral-50 text-xs font-normal font-['Poppins']">
                Note: This action cannot be undone. You can edit this {entityName} instead of permanently deleting it.
              </div>
            </div>
            {error && (
              <div className="w-full text-center text-sm text-red-400 mt-2">{error}</div>
            )}
          </div>
          <div className="self-stretch inline-flex justify-center items-start gap-3">
            ...
            >
              <div className="text-center justify-start text-neutral-50 text-sm font-medium font-['Inter'] leading-5">
                Cancel
              </div>
            </div>
            <div
              ...
              data-button="on"
              data-size="Large"
              className={`flex-1 px-4 py-2 bg-neutral-50 rounded-full flex justify-center items-center gap-2 ${
                loading ? "opacity-60 cursor-not-allowed" : "cursor-pointer"
              }`}
              onClick={(): void => {
              ...
              aria-disabled={loading}
            >
              <div className="text-center justify-start text-zinc-950 text-sm font-medium font-['Inter'] leading-5">
